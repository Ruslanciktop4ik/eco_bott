...
@bot.command()
async def helpanimals(ctx):
    helpanimals = ["Помогите бездомным животным, если вы видитеживотное, которое вы не можете забрать с собой, оставьтеводу и еду в безопасном месте, чтобы животное моглопитаться и пить",
                   "Что бы помочь бездомным животным вы можете пожертвовать корм, деньги и т.д в приюты",
                   "Что бы помочь бездомным животным вы можете забрать его из приюта к себе домой",
                   "Что бы помочь животным вы можете стать волонтером фонда",
                   "Рекламируйте приюты в соцсетях что бы другие люди взяли от туда себе питомца",
    ]
    await ctx.send(f'Рандомный совет:'{random.choice(helpanimals)})
bot.run('')
